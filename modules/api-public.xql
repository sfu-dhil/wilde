...
declare namespace api = "http://dhil.lib.sfu.ca/exist/wilde/api-public";
    ...
    <root>
      ...
      }
      {
        ...
        let $target := collection:fetch($link/@href/string())
          order by document:date($link),
            document:date($target),
            $link/@data-similarity descending
        return
          if ((document:date($link) = document:date($target)) and (document:id($link) gt document:id($target))) then
            ()
          else
            if (document:date($link) gt document:date($target)) then
              ()
            else
...
declare function api:sources() {
  ...
  return
    <json:value>{$city}</json:value>
};

let $functionName := request:get-attribute('function')
let $function :=
try {
  function-lookup(QName("http://dhil.lib.sfu.ca/exist/wilde/api-public", $functionName), 0)
} catch * {
  ()
}
...
return
  if (exists($function)) then
    ...
    return
      <error status="404">The API function {$functionName} cannot be found.</error>
